""" 18. Uma grande emissora de televisão quer fazer uma enquete entre os
seus telespectadores para saber qual o melhor jogador após cada jogo.
...
contratada para desenvolver este programa, utilizando a linguagem de programação C++.
...
indica que a votação foi encerrada. Se um número inválido for digitado,
...
Ao final, o programa deve ainda gravar os dados referentes ao resultado da
votação em um arquivo texto no disco, obedecendo a mesma disposição apresentada na tela.
...
def porc_func(numero, totalvoto):
    totalvoto = len(totalvoto)
    numerocont = votos.count(numero)
    porcentagem = numerocont/totalvoto * 100
...
if __name__ == '__main__':
    ...
    print('Foram computados', len(votos), 'votos.\n')

    print('{:<8} {:<14} {:<8}'.format('Jogador', 'Votos', '%'))

    lines = [f'Resultado da votacao:\nForam computados {len(votos)} votos.\nJogador  Votos          %']

    for n in sorted(votos):
        if n not in duplicados:
            a = '{:<10} {:<10} {:<1} {:<10}'.format(n, votos.count(n), porc_func(n, votos), '%')
            duplicados.add(n)
            print(a)
            lines.append(a)

    lines.append(f'O melhor jogador foi o numero {max(votos, key=votos.count)}, com '
                 f'{votos.count(max(votos, key=votos.count))} '         
                 f'votos, correspondendo a {"{:.1f}".format(votos.count(max(votos, key=votos.count))/len(votos) * 100)}'
                 f'% do total de votos.')

    print(f'O melhor jogador foi o numero {max(votos, key=votos.count)}, '
          f'com {votos.count(max(votos, key=votos.count))} '
          f'votos, correspondendo a {"{:.1f}".format((votos.count(max(votos, key=votos.count))/len(votos)) * 100)}'
          f'% do total de votos.')
